...
生产环境可启用 Workers AI 作为在线 Provider：提示词包含最近 30 天的句子、词条与场景表达摘要。模型只返回受约束的核心学习字段，服务端再补全场景迁移提示、搭配和用法说明；这样能显著减小结构化响应，避免大对象被截断，同时最终候选仍需经过完整 schema、敏感主题、版权声明、XSS、语义哈希和相似度验证。模型响应不保证始终满足 JSON Schema，因此系统最多生成三个候选并逐个验证。Provider 失败时回退种子；如果在线候选与内置种子都无法提供新内容，系统明确失败并等待下一次有限重试，不会为了“发送成功”静默复用旧内容。
