...
每个 entry 会在音标后生成中文释义摘要。摘要按词性聚合、去重并保留全部中文释义，每个词性独占一行、每行以中文分号结束，例如 `n.施乐（品牌名）；影印；` 与下一行 `vt.复印；`。来源明确提供 `vi.`、`vt.` 或 `aux.` 时保留该区别；只有来源未区分时才显示 `v.`，不凭猜测改写词性。摘要只调整阅读顺序，不删除或改写后续逐义项内容。
...
Provider 音频不会再由浏览器跨站直连。API 将经过主机和路径白名单验证的音频地址改写为同源 `GET /api/dictionary/audio`，Worker 验证 `audio/*`、大小和 Range 后流式转发；这样避免第三方防盗链、重定向或浏览器跨源策略导致“有播放器但没有声音”。播放失败时按钮会明确提示，并尝试设备内置英语发音。
...
合并后的所有 entries、词性和 senses 会先完整保留，再由服务端 Workers AI 以受 JSON Schema 约束的批次补充每条中文释义和已有来源例句的中文翻译；批次异常时再使用 `m2m100-1.2b` 做有限逐条回退。每批最多 24 项、最多四个批次并行，减少首次多义词查询等待时间。结果写入 D1 翻译缓存，相同英文只翻译一次。UI 以“中文补充”单独展示，原英文定义、例句、层级、顺序和来源不被改写；没有例句时仍显示“暂无来源例句”。翻译失败时返回明确 503，不以截断部分义项伪装完整结果。

生产完整性检查使用高频多义词 `run`：合并后仍返回形容词、名词、动词 3 类词性、120 个义项和 125 条来源例句，全部义项与例句均有中文补充。首次查询耗时取决于 Provider 与 Workers AI；后续相同英文会命中 D1 翻译缓存。
...
词典搜索框首次打开保持为空，不自动请求示例词。页面在搜索框下方始终显示当前账号的“我的收藏”，收藏词条可直接点击重新查询；最近搜索仅在确有历史时出现。
